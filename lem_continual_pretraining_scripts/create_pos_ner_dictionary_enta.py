...
#v2 - ner only
#v3 - ner, new_wo_nums
#v4 - ner, new_wo numbers, pos for mlm experiments
...
input_file_names = ["en_ta_train_tlm.txt", "en_ta_valid_tlm.txt"]
data_out_subdir="data_out"
out_file_name="ne_pos-tlm-dict_v5.en-ta.pkl"
src="en"
tgt='ta'
...
ta_pos_dict={}
with open(ta_pos_dict_path, 'rb') as pos_file:
    ta_pos_dict = pickle.load(pos_file)
print('Total entries in the Ta ne_pos_dict : {}'.format(len(ta_pos_dict)))
...
def pos_tag_sequence(sentence_words):
    ...
    for i in range(0, len(sentence_words)):
        ...
        elif sentence_words[i] in ta_pos_dict:
            pos_tags_with_tokens.append('[{}]{}/{}'.format(i, sentence_words[i], ta_pos_dict[sentence_words[i]]))
            pos_tags.append(ta_pos_dict[sentence_words[i]])

            if ta_pos_dict[sentence_words[i]]=="VERB":
                verb_indexes.append(i)
            elif ta_pos_dict[sentence_words[i]]=="NOUN":
                noun_indexes.append(i)
...
for input_file_name in input_file_names:
    for index, sentence_text in enumerate(open("{}/{}".format(data_dir, input_file_name), "r", encoding="utf8")):
