...
input_file_names = ["si_ta_train_tlm.txt", "si_ta_valid_tlm.txt"]
data_out_subdir="data_out"
out_file_name="ne_pos-tlm-dict_v5.si-ta.pkl"
src="si"
tgt='en'
...
si_pos_dictionary_file="/userdirs/aloka/p31_pretrained_models_ft/4_ner_pos_exp/ner_pos_dicts/train100KDD-tu.si.pos-all-words.dict"
si_pos_tag_dict={}

with open(si_pos_dictionary_file, 'rb') as ner_pos_file:
    si_pos_tag_dict = pickle.load(ner_pos_file)
print('Total entries in the ne_pos_dict : {}'.format(len(si_pos_tag_dict)))


#pos_tag_dict path---------------
ta_pos_dict_path="/userdirs/aloka/nlpc_tools/tamil_pos_tagger/train100KDD-tu-pos-ta.pkl"
ta_ne_pos_dict={}

with open(ta_pos_dict_path, 'rb') as ner_pos_file:
    ta_ne_pos_dict = pickle.load(ner_pos_file)
print('Total entries in the ne_pos_dict : {}'.format(len(ta_ne_pos_dict)))
...
def pos_tag_sequence(sentence_words):
    ...
    for i in range(0, len(sentence_words)):
        ...
        if sentence_words[i] in si_pos_tag_dict:
            pos_tags_with_tokens.append('[{}]{}/{}'.format(i, sentence_words[i], si_pos_tag_dict[sentence_words[i]]))
            pos_tags.append(si_pos_tag_dict[sentence_words[i]])

            if si_pos_tag_dict[sentence_words[i]] in si_verb_tags:
                verb_indexes.append(i)
            elif si_pos_tag_dict[sentence_words[i]] in si_noun_tags:
                noun_indexes.append(i)

        elif sentence_words[i] in ta_ne_pos_dict:
            pos_tags_with_tokens.append('[{}]{}/{}'.format(i, sentence_words[i], ta_ne_pos_dict[sentence_words[i]]))
            pos_tags.append(ta_ne_pos_dict[sentence_words[i]])

            if ta_ne_pos_dict[sentence_words[i]]=="VERB":
                verb_indexes.append(i)
            elif ta_ne_pos_dict[sentence_words[i]]=="NOUN":
                noun_indexes.append(i)
